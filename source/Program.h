...
struct ProgramDesc
{
    ...
    {
        if (raygen.empty()) {
            assert(!"'raygen' entry point name must not be empty");
        }
    ...
    {
        if(miss.empty()) {
            assert(!"'miss' entry point name must not be empty");
        }
    ...
    {
        if (closestHit.empty() && anyHit.empty() && intersection.empty() ) {
            assert(!"At least one of 'closestHit', 'anyHit' or 'intersection' entry point names must not be empty");
        }
